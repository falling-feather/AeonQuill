...
- 主 NSIS 不直接塞入超过 50 GiB 的 ComfyUI/Python/模型树；完整离线交付采用“应用安装器 + 可校验运行时目录 + 一键安装脚本”的拆分包。默认原位使用完整包运行时，移动、删改目录或校验失败会阻止安装/执行。
...
- 完整离线布局将应用二进制与 `UserData` 分开，卸载应用不删除用户项目；旧数据迁移仅由本地安装脚本显式执行，仍无自动在线迁移或回滚服务。
