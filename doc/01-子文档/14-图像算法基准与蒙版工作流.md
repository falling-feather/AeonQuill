...
- 尚未对 u2netp、BiRefNet、InSPyReNet、IS-Net 或 SAM 辅助流运行同批比较。
...
| SAM v1 ViT-B | 点击/框选辅助蒙版 | 不与全自动去背景混成同一体验；复用本机 ComfyUI 前需单独适配 |
...
- `npm run test:unit` 通过 55/55，其中 9 项直接覆盖蒙版配方与图像质量协议。
...
- 浏览器真实验证了绘制后 Alpha 改变、撤销/重置启用、应用按钮启用、派生节点/连接线创建、服务端外置和刷新恢复。
...
3. 为 SAM 增加点击/框选提示与独立辅助流，不把它当作零提示全自动去背景。
