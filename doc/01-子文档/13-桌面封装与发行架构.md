...
> **文档梗概**：记录 Electron/Tauri 壳层决策、AEONQUILL Windows x64 安装与生命周期契约，以及 V0.4 完整 ComfyUI/Python/H3 离线运行时、非系统盘便携布局、V0.5.0 应用更新和 V0.5.1 框选补丁实装证据。
>
> **具体规划法则**：
>
...
用户项目、配置和模型位于应用数据目录或用户显式选择的位置，不放入安装目录。桌面壳默认在应用本地数据根目录下建立 `runtime/`、`data/`、`cache/`、`logs/` 与 `config/`；便携布局还在安装根建立与 `App`、`UserData` 同级的 `output` 交付目录。桥接在新 data 为空而旧 runtime 存在 jobs/projects/assets/client-state 时继续使用旧持久数据并公开 `LEGACY_DATA_LAYOUT`，不静默搬移或删除。卸载器只移除应用二进制和登记，默认保留用户数据、output 及外部 ComfyUI/模型。内部 identifier 仍沿用兼容值，待后续迁移任务完成后再切换。
...
Tauri 渲染器只拥有 `core:default` 与原生输出目录选择所需的 `dialog:allow-open` capability，`withGlobalTauri` 为 false，不暴露 Shell permission；导航限制到精确桥接 Origin，新窗口禁止。页面实际探针确认 `window.__TAURI__`、`process` 与 `require` 均不可见。
...
`REL-008` 的当前安装包为 `AEONQUILL_0.4.5_x64-setup.exe`，243,336,663 bytes，SHA-256 `a0a3173946198801464d6468b938113bf5c82cee705d3a1b8257c91571658471`；release manifest 绑定干净源码 `9c4b0f4de189e4e547e660b47d0e58aeb1495f40`，6/6 文件校验通过，Authenticode 为 `NotSigned`。D 盘隔离安装—隐藏启动—卸载和正式 `D:/AEONQUILL/App` 隐藏生命周期均通过，当前注册版本为 0.4.5；50.28 GiB 运行时和 `UserData` 未进入应用更新包。NSIS 载荷把主程序的 Tauri bundle marker 从 `UNK` 改为 `NSS`，与 staging 中构建组件仅有三个字节差异；因此发行完整性以安装器 SHA-256 为边界，构建组件哈希和安装态主程序哈希分别记录，不要求二者相等。

`REL-009` 的当前安装包为 `AEONQUILL_0.5.0_x64-setup.exe`，243,356,183 bytes，SHA-256 `415b48abed2c1ab0c944a3be5aa2d1ce17b5ce5bc5871a029f3a98580f698285`；release manifest 绑定干净源码 `88aeb8f59994cbee34ad9f4607284d8dcdf405d3`，6/6 文件校验通过，Authenticode 为 `NotSigned`。D 盘隔离安装/启动/卸载为 18.537/2.693/6.865 秒；正式 `D:/AEONQUILL/App` 已登记 0.5.0，安装态窗口 1.710 秒加载并优雅关闭，包内 sidecar 返回 H3 多场景/提示词编排目录并识别软件默认 output。应用自动创建 `D:/AEONQUILL/output`；用户配置与 SQLite 主库/WAL/SHM 哈希更新前后不变，唯一桌面快捷方式指向新程序，C 盘旧 profile、desktop/sidecar/8188/8787 均无残留。安装态主程序的 NSIS bundle marker 仍会令哈希不同于 staging 构建组件，因此两者继续分别记录。

`REL-010` 从干净提交 `4e7e76f0381c823fdf0ab969af09969984036bd9` 生成 `AEONQUILL_0.5.1_x64-setup.exe`，243,356,361 bytes，SHA-256 `7239b241ed997fbddd840e1bde3d5ba3912febd830ab896aa06ee9864a5ae2fb`；manifest dirty=false、6/6，Authenticode 为 `NotSigned`。sidecar 在 system-only PATH 494ms 就绪，release/installed 窗口 1.797/1.795 秒加载并经真实关闭请求优雅退出。正式空闲应用以隐藏 NSIS 原位更新 23.612 秒，主程序 ProductVersion、注册表、唯一桌面快捷方式描述均为 0.5.1；安装态主程序/sidecar SHA-256 为 `1ff74ec90b9d8a71c85d61d7e8d35e9c604f72c8287e55b7bea7ab458d524d9f` / `168f32e322864bd1b63514a7ceb3db2cc226a4279fb588adc43cc78641ebabd9`。配置与 SQLite 主库/WAL/SHM 哈希不变，8188/8787 和应用进程无残留；误触发 debug QA 产生的 11.43 MiB C 盘临时 WebView2 profile 已核对无持久数据并移入回收站，D 盘活动 profile 保留。
...
开发机需要 Node.js 22。Tauri 构建还需要本机 MSVC/WebView2，并通过项目脚本加载 `.runtime/toolchains/rust` 中的项目级 Rust 工具链。`bundle.useLocalToolsDir=true` 将 NSIS 等封包工具缓存固定到项目 `target/.tauri`（工作区位于 D 盘），避免后续构建重新占用 `%LOCALAPPDATA%/tauri`；该目录是可再生产物并被 Git 忽略。
...
`package:desktop:tauri` 依次生成生产前端、带固定工作流 JSON 的 Node 22 sidecar、Tauri release、NSIS 和受控 release staging；所有文件名均由 `package.json` 产品 SemVer 与 Tauri 产品名动态生成。`validate:desktop:release` 会短暂创建隔离的当前用户安装记录并立即卸载；它拒绝覆盖已有 AEONQUILL 开始菜单、桌面快捷方式或卸载登记，并断言真实主窗口关闭会退出应用/sidecar/端口、卸载后用户数据哨兵仍存在。构建态 Tauri QA 会把主程序与 sidecar 复制到 D 盘临时 `App/UserData` 便携布局后运行；已有安装态可用 `node scripts/validate-desktop-tauri.mjs --installed-directory=<App>` 直接核验，从而不再让 WebView2 测试 profile 回落 `%LOCALAPPDATA%`。正式候选必须从干净提交重建并通过 `--require-clean`。

完整离线包不是单个巨型 NSIS。`stage:desktop:runtime` 根据 `desktop/runtime/runtime-lock.json` 只复制登记的 ComfyUI 核心、两个 Custom Nodes、模型、便携 Python 与三类图片工具，明确排除 input/output/temp/user、缓存、密钥、未知节点和未知权重；生成 `runtime-manifest.json` 与覆盖 63,232 文件的 `runtime-SHA256SUMS.txt`。`stage:desktop:full-offline` 再把干净主安装器、发行说明、安装脚本和版本化运行时组合到 `.runtime/releases/offline/AEONQUILL_<version>_windows_x64_full/`，根 `offline-release-manifest.json` 绑定主安装器、脚本与运行时清单。

V0.4.1 双击入口默认使用 `-UsePayloadInPlace`，以完整包所在磁盘为安装盘并创建 `AEONQUILL/App` 与同级 `UserData`，因此不会把 50.28 GiB 运行时复制到 C 盘。安装器在 `App/aeonquill-layout.json` 写入只有三个固定字段的布局标记；Tauri/Rust 与 Electron/Node 均拒绝未知字段、未知布局和任意数据目录名，只允许解析到安装根同级 `UserData`。环境变量仍可覆盖该默认值以支持隔离 QA。`-MigrateLegacyData` 迁移旧 app-local-data，`-RemoveLegacyAfterMigration` 明确授权后才移动并清理空的旧根；V0.4.2 进一步把移动操作已经删除旧根视为正常成功，避免数据已迁移后因二次删除中断。V0.4.3 还通过 Tauri 的 `data_directory` 把 WebView2 profile 固定到 `UserData/EBWebView`，避免内部兼容 identifier 令浏览器缓存回落 C 盘。`-ReplaceExistingApplication` 只调用登记位置内的旧 AEONQUILL 卸载器。只有 `-RepairRuntime` 才能删除经过父目录/精确目标验证的同版本运行时。
...
上述二进制、50.28 GiB 运行时、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-004` 已从 staged 运行时真实完成包内 ComfyUI 冷启动、7/7 图片处理器及固定 T2V/I2V；`REL-005`—`REL-008` 已完成便携布局、C 盘收束、体验修复和 0.4.5 实装；`REL-009` 从干净 V0.5.0 候选源重建并实装应用本体；`REL-010` 从干净 V0.5.1 候选源交付框选修复、完成正式 D 盘原位更新、数据哈希保留、唯一快捷方式与安装态生命周期复核。当前只能交付未签名本机阶段包；正式公开发行仍必须完成签名、更新/回滚、完整第三方 SBOM/许可与地域合规、内容安全和扩大干净机/驱动矩阵。
