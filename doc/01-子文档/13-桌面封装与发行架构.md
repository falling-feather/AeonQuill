...
> **文档梗概**：记录 `REL-001` 的 Electron/Tauri 对照实验、`REL-003` 对 AEONQUILL Windows x64 阶段测试包的身份与发行契约收束，以及 `QA-003` 对唯一 V0.3 候选的独立安装验收结论。
>
> **具体规划法则**：
>
...
Windows 包采用 NSIS 当前用户安装，WebView2 模式为 `embedBootstrapper`。安装器、窗口、主程序、sidecar、开始菜单目录、快捷方式和卸载登记统一为 AEONQUILL；应用图标采用 Q 与羽笔组合标识。安装器把安装说明、限制与第三方声明放入 `release/` 资源目录。隔离验收覆盖静默安装、安装版启动、五类数据目录、桥接退出、干净 data 建库、旧 runtime 持久数据回退、两类用户数据保留和静默卸载，并确认程序文件、卸载注册项和开始菜单快捷方式均清理。`DEV-014` 后自动验收不再直接调用 `app.exit()`：QA WebView 发出真实 `window.close()`，报告必须存在 `windowCloseRequestedMs`，且 sidecar 优雅退出与端口关闭后应用才算通过。
...
- 主包不捆绑 ComfyUI、Python、Custom Nodes、模型或 API Key；许可证、SBOM 和商业再分发结论尚未完成。
...
`package:desktop:tauri` 依次生成生产前端、Node 22 sidecar、Tauri release、NSIS 和受控 release staging；所有文件名均由 `package.json` 产品 SemVer `0.3.0` 与 Tauri 产品名动态生成，候选源码 Git 任务版本 `V0.3.4` 和 QA 见证版本 `V0.3.5` 都不改写产品版本。`validate:desktop:release` 会短暂创建一个隔离的当前用户安装记录并立即卸载；它拒绝覆盖已有 AEONQUILL 快捷方式或卸载登记，并断言真实主窗口关闭会退出应用/sidecar/端口、卸载后正常 data 与旧 runtime 回退数据哨兵仍然存在。正式候选 staging 必须由干净提交重建并通过 `--require-clean`，使 manifest 中 `source.gitCommit` 等于候选提交且 `workingTreeDirty=false`。默认 `npm run validate` 不执行桌面安装或启动基准，以保持日常回归快速且无注册表变更。
...
上述二进制、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-003` 形成的候选在两轮缺陷修复后已由 `QA-003` 独立验收通过，`V0.3` 因此按阶段测试安装包口径退出。当前能交付的仍只是未签名阶段测试包；正式公开发行仍必须完成签名、完整第三方许可证/SBOM 与商业再分发核对、扩大干净机兼容矩阵和更新回滚门禁。
