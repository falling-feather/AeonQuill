...
> **文档梗概**：记录 Electron/Tauri 壳层决策、AEONQUILL Windows x64 安装与生命周期契约，以及 `REL-004` 的 V0.4 主安装器 + 完整 ComfyUI/Python/H3 离线运行时拆分发行方案。
>
> **具体规划法则**：
>
...
完整离线包不是单个巨型 NSIS。`stage:desktop:runtime` 根据 `desktop/runtime/runtime-lock.json` 只复制登记的 ComfyUI 核心、两个 Custom Nodes、模型、便携 Python 与三类图片工具，明确排除 input/output/temp/user、缓存、密钥、未知节点和未知权重；生成 `runtime-manifest.json` 与覆盖 63,232 文件的 `runtime-SHA256SUMS.txt`。`stage:desktop:full-offline` 再把干净主安装器、发行说明、安装脚本和版本化运行时组合到 `.runtime/releases/offline/AEONQUILL_0.4.0_windows_x64_full/`，根 `offline-release-manifest.json` 绑定主安装器、脚本与运行时清单。

一键脚本默认把运行时复制到当前用户应用数据的 `runtime/packages/<packageId>`；`-UsePayloadInPlace` 允许直接使用交付目录。脚本安装前核验根清单聚合哈希、所有绑定文件及关键模型/执行器，并要求用户明确接受 MiniMax H3 条款。只有 `-RepairRuntime` 才能删除经过父目录/精确目标验证的同版本运行时，卸载应用不会删除用户项目或运行时。
...
上述二进制、50.28 GiB 运行时、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-004` 已从 staged 运行时真实完成包内 ComfyUI 冷启动、7/7 图片处理器及固定 T2V/I2V；最终安装器仍须从本提交后的干净源码重建并完成整包安装验收。当前只能交付未签名本机阶段包；正式公开发行仍必须完成签名、更新/回滚、完整第三方 SBOM/许可与地域合规、内容安全和扩大干净机/驱动矩阵。
