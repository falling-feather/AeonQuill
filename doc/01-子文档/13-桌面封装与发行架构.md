...
`package:desktop:tauri` 依次生成生产前端、带固定工作流 JSON 的 Node 22 sidecar、Tauri release、NSIS 和受控 release staging；所有文件名均由 `package.json` 产品 SemVer 与 Tauri 产品名动态生成。`validate:desktop:release` 会短暂创建隔离的当前用户安装记录并立即卸载；它拒绝覆盖已有 AEONQUILL 快捷方式或卸载登记，并断言真实主窗口关闭会退出应用/sidecar/端口、卸载后用户数据哨兵仍存在。正式候选必须从干净提交重建并通过 `--require-clean`。
...
上述二进制、50.28 GiB 运行时、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-004` 已从 staged 运行时真实完成包内 ComfyUI 冷启动、7/7 图片处理器及固定 T2V/I2V；`REL-005`/`REL-006` 已完成便携布局和迁移幂等修复；`REL-007` 已从干净 V0.4.3 提交重建，完成 WebView2 D 盘 profile、隔离安装/卸载、真实 D 盘替换、安装态 API 与 C 盘收尾。当前只能交付未签名本机阶段包；正式公开发行仍必须完成签名、更新/回滚、完整第三方 SBOM/许可与地域合规、内容安全和扩大干净机/驱动矩阵。
