...
> **文档梗概**：记录 `REL-001` 的 Electron/Tauri 对照实验，以及 `REL-003` 对 AEONQUILL Windows x64 阶段测试包的身份、运行目录、sidecar、安装卸载、发布 manifest 和回退壳契约收束。
>
> **具体规划法则**：
>
...
`V0.3` 采用 **Tauri 作为 Windows x64 阶段测试包的主发行壳，Electron 作为可随时回退的参考壳**。该选择只针对当前单机、本地优先、希望减小下载体积的产品形态：

- AEONQUILL Tauri 已生成并真实验证约 27.7 MiB 的 NSIS 安装包；Electron 回退壳当前为 349.4 MiB 未压缩便携目录。
...
2. 以环境白名单启动桥接，注入 `AEONQUILL_HOST`、`AEONQUILL_PORT`、`AEONQUILL_RUNTIME_DIR`、`AEONQUILL_DATA_DIR`、`AEONQUILL_CACHE_DIR`、`AEONQUILL_LOG_DIR` 与 `AEONQUILL_CONFIG`；在兼容期同步镜像既有 `MIAOHUI_HOST`、`MIAOHUI_PORT`、`MIAOHUI_RUNTIME_DIR` 与 `MIAOHUI_CONFIG`。ComfyUI 策略和空闲秒数仅在父环境明确提供时转发，不用桌面默认值覆盖 `config/local.json` 中的用户选择。
...
Windows 包采用 NSIS 当前用户安装，WebView2 模式为 `embedBootstrapper`。安装器、窗口、主程序、sidecar、开始菜单目录、快捷方式和卸载登记统一为 AEONQUILL；应用图标采用 Q 与羽笔组合标识。安装器把安装说明、限制与第三方声明放入 `release/` 资源目录。隔离验收已覆盖静默安装、安装版启动、五类数据目录、桥接退出、干净 data 建库、旧 runtime 持久数据回退、两类用户数据保留和静默卸载，并确认程序文件、卸载注册项和开始菜单快捷方式均清理。
...
`package:desktop:tauri` 依次生成生产前端、Node 22 sidecar、Tauri release、NSIS 和受控 release staging；所有文件名均由 `package.json` 产品 SemVer `0.3.0` 与 Tauri 产品名动态生成，Git 任务版本 `V0.3.2` 不改写产品版本。`validate:desktop:release` 会短暂创建一个隔离的当前用户安装记录并立即卸载；它拒绝覆盖已有 AEONQUILL 快捷方式或卸载登记，并断言卸载后正常 data 与旧 runtime 回退数据哨兵仍然存在。正式候选 staging 必须由干净提交重建并通过 `--require-clean`，使 manifest 中 `source.gitCommit` 等于候选提交且 `workingTreeDirty=false`。默认 `npm run validate` 不执行桌面安装或启动基准，以保持日常回归快速且无注册表变更。
...
上述二进制、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-003` 已形成可交给 `QA-003` 的本机验证候选；在独立 QA 完成前仍不宣布 `V0.3` 周期退出。当前能交付的也只是未签名阶段测试包；正式公开发行仍必须完成签名、完整第三方许可证/SBOM、扩大干净机兼容矩阵和更新回滚门禁。
