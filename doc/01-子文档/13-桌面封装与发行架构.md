...
> **文档梗概**：记录 Electron/Tauri 壳层决策、AEONQUILL Windows x64 安装与生命周期契约，以及 V0.4 完整 ComfyUI/Python/H3 离线运行时、非系统盘便携布局、V0.5.0 应用更新证据和 V0.5.1 补丁候选边界。
>
> **具体规划法则**：
>
...
`REL-010` 已把源码与发行版本镜像提升到产品 `0.5.1`，用于交付 `DEV-016` 的框选预览定位修复。候选仍只更新应用本体并复用既有 D 盘运行时；在安装器、manifest、安装态生命周期、用户数据哈希、快捷方式和真实框选回归全部完成前，正式 `D:/AEONQUILL/App` 仍按 0.5.0 记录，不得把版本镜像变更写成已安装事实。
...
上述二进制、50.28 GiB 运行时、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-004` 已从 staged 运行时真实完成包内 ComfyUI 冷启动、7/7 图片处理器及固定 T2V/I2V；`REL-005`—`REL-008` 已完成便携布局、C 盘收束、体验修复和 0.4.5 实装；`REL-009` 从干净 V0.5.0 候选源重建应用本体，完成隔离安装—启动—卸载、真实 D 盘 0.5.0 替换、默认 output、安装态 H3 API、数据哈希保留、唯一快捷方式与生命周期复核。当前只能交付未签名本机阶段包；正式公开发行仍必须完成签名、更新/回滚、完整第三方 SBOM/许可与地域合规、内容安全和扩大干净机/驱动矩阵。
