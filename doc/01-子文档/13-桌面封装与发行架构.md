...
上述二进制、50.28 GiB 运行时、`.runtime/` 和 `target/` 都是可再生构建产物，不进入 Git。`REL-004` 已从 staged 运行时真实完成包内 ComfyUI 冷启动、7/7 图片处理器及固定 T2V/I2V；`REL-005` 已完成便携布局代码与单元/Rust 契约，`REL-006` 正从干净 V0.4.2 提交重建并完成 D 盘整包安装验收。当前只能交付未签名本机阶段包；正式公开发行仍必须完成签名、更新/回滚、完整第三方 SBOM/许可与地域合规、内容安全和扩大干净机/驱动矩阵。
